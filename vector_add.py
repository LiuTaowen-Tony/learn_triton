...
def add_kernel(x_ptr,  # *Pointer* to first input vector.
    ...
    for i in range(tl.cdiv(BLOCK_SIZE, 32)):
        offsets = block_start + tl.arange(0, 32) + i * 32
...
def benchmark(M, N, provider):
    ...
    if provider == 'torch-jit':
        ms, min_ms, max_ms = triton.testing.do_bench(lambda: torch.add(x), quantiles=quantiles)

...
@triton.jit
def _int8_matmul_rowwise_dequantize_slow(A, B, C, bias, state_x_ptr, state_w_ptr, M, N, K, divfactor, has_bias : tl.constexpr,
            stride_am, stride_ak,
            stride_bk, stride_bn,
            stride_cm, stride_cn,
...
def int8_matmul_rowwise_dequantize_slow(a, b, state_x, state_w, bias):
    divfactor = 1. / (127. * 127.)
    ...
    _int8_matmul_rowwise_dequantize_slow[grid](a, b, c, bias, state_x, state_w, M, N, K, divfactor, has_bias,
...
def matmul_slow(a, b, state_x=None, state_w=None, bias=None):
    if state_x is None:
        a, state_x = quantize_rowwise(a)
    if state_w is None:
        b, state_w = quantize_rowwise(b)
    return int8_matmul_rowwise_dequantize_slow(a, b.t(), state_x, state_w, None)
...
def matmul(a, b, state_x=None, state_w=None, bias=None):
    ...
    if state_w is None:
        b, state_w = quantize_rowwise(b)
    return int8_matmul_rowwise_dequantize(a, b.t(), state_x, state_w, None)
...
def benchmark(M, N, K, provider):
    ...
    if provider == 'triton_slow':
        state_x = None
        state_w = None
        # a, state_x = quantize_rowwise(a)
        # b, state_w = quantize_rowwise(b)
        b = b.t()
        ms, min_ms, max_ms = triton.testing.do_bench(lambda: matmul_slow(a, b, state_w, state_x), quantiles=quantiles) 
